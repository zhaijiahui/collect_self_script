...
'Mozilla/5.0 (iPhone; CPU iPhone OS 10_2_1 like Mac OS X; zh-CN) AppleWebKit/537.51.1 (KHTML, like Gecko) Mobile/14D27 UCBrowser/11.6.1.1003 Mobile  AliApp(TUnionSDK/0.1.20)']
    ...
    try:
        r = requests.get('http://'+ip +'/.svn/entries',headers=headers,timeout=3,verify=False)
        html = r.text
        if r.status_code == 200:
            if 'dir' in html:
                print('Find: ' + 'http://'+ip +'/.svn/entries' +' is Leak !!!')
            else:
                print('Find: ' + 'http://'+ip +'/.svn/entries' +' is Exist !!!')
    except Exception as e:
        try:
            r = requests.get('https://'+ip +'/.svn/entries',headers=headers,timeout=3,verify=False)
            html = r.text
            if r.status_code == 200:
                if 'dir' in html:
                    print('Find: ' + 'https://'+ip +'/.svn/entries' +' is Leak !!!')
                else:
                    print('Find: ' + 'https://'+ip +'/.svn/entries' +' is Exist !!!')
        except Exception as e:
            pass
    

def main():
    ipl = iplist('111.205.243.0','111.205.243.255') # change start ip and end ip
